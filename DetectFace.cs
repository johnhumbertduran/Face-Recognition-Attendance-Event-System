...
namespace Face_Recognition_Attendance_Event_System
{
    public partial class DetectFace : Form
    {
        ...
        {
            connection.Open();
            string check_number = "SELECT * FROM userstbl WHERE student_number='" + textBox1.Text + "' AND reg_check='2' ";
            ...
            if (textBox1.Text != "")
            {
                ...
                //MessageBox.Show(dr.GetString(2) + " " + dr.GetString(3));
                if (read3.Read() == true)
                {
                    textBox2.Text = read3.GetString(2) + " " + read3.GetString(3);
